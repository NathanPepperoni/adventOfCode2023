...
function getCategory(hand) {
  ...
  let uniqueCount = 0;
  for (let i = 0; i < 5; i++) {
    if (hand.cards[i] !== 1 && arr[hand.cards[i] - 1] === 0) {
      uniqueCount++;
    }
    arr[hand.cards[i] - 1] += 1;
  }

  const jokerCount = arr[0];

  if (uniqueCount <= 1) {
    return Type.FIVE_OF_KIND;
  }
  if (uniqueCount === 2) {
    const maxCount = getMaxValue(arr);
    return maxCount + jokerCount === 4 ? Type.FOUR_OF_KIND : Type.FULL_HOUSE;
  }
  if (uniqueCount === 3) {
    const maxCount = getMaxValue(arr);
    return maxCount + jokerCount === 3 ? Type.THREE_OF_KIND : Type.TWO_PAIR;
  }
  if (uniqueCount === 4) {
